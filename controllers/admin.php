...
class Admin extends Admin_Controller
{
    ...
    public function parse($file)
    {
        ...
        foreach($obj as $table_name => $table_data)
        {
            ...
            if(!strstr($table_name,"ci_sessions"))
            {
                ...
                if (!$this->db->table_exists($table_name) && $table_name != "cms_version" && $table_name != "site_ref" && $table_name != "cms_edition" && !strstr($table_name,"_table_columns"))
                {
                    $forge_data = $this->get_table_key_column_types($obj,$table_name);
                    if($forge_data)
                    {
                        // Empty prefix
                        $this->db->set_dbprefix('');

                        foreach($forge_data as $forge_data_key => $forge_data_value)
                        {
                            if($forge_data_value['auto_increment'] == "true")
                            {
                                $this->dbforge->add_key($forge_data_key, TRUE);
                            }
                        }

                        $this->dbforge->add_field($forge_data);
    ...
    public function get_table_key_column_types($obj,$table)
    {
        $columns_table_name = $table . "_table_columns";
        foreach($obj as $table_name => $table_data)
        {
            if($table_name == $columns_table_name)
            {
                foreach($table_data as $k => $v)
                {
                    if((string) $v->type ==  "varchar" || (string) $v->type ==  "int")
                    {
                        $fields[(string) $v->name]['type'] = strtoupper((string) $v->type . "(".(string) $v->max_length.")");
                    }
                    else
                    {
                        $fields[(string) $v->name]['type'] = strtoupper((string) $v->type);
                    }

                    if((string) $v->default != "")
                    {
                        $fields[(string) $v->name]['default'] = (string) $v->default;
                    }

                    $fields[(string) $v->name]['auto_increment'] = ((string) $v->primary_key == "1" ? true : false);
                }
            }
        }
        if(count($fields) > 0)
